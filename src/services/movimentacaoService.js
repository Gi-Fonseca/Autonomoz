...
class MovimentacaoService {
    ...
    async cadastrar(dados) {
        ...
        if (!id_entrada && !id_saida) {
            const erro = new Error("A movimentação deve ter pelo menos um 'id_entrada' OU 'id_saida'.");
            erro.status = 400;
            throw erro;
        }
    ...
    async atualizar(id, dados) {
        ...
        if (!id_estoque || (!id_entrada && !id_saida)) {
            const erro = new Error("Informe 'id_estoque' e pelo menos 'id_entrada' ou 'id_saida'.");
            erro.status = 400;
            throw erro;
        }
